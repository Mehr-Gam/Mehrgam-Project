...
import {
  ...
  findAcceptedRequestsForVolunteer,
...
export const getAcceptedRequestsForVolunteer = async (user) => {
  ensureVolunteerRole(user);

  const requests = await findAcceptedRequestsForVolunteer(user.volId);
